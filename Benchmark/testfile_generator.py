...
def generate_template(target_dir="Benchmark", filename="test_template.py"):
   
    # Template content with placeholders for `state` and `environment`
    content = f"""
import os
import time
import pytest
...
def starter(request):
    ...
    # Full file path for the template
    full_path = os.path.join(target_dir, filename)

    # Write the generated content to the specified file
    with open(full_path, 'w') as f:
        f.write(content)
    print(f"Template generated in {filename}")



# Generate the template file
generate_template(target_dir="Benchmark", filename="test_template.py")
