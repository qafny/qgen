...
class QProgram(TreeProgram):
    ...
    @staticmethod
    def get_match_indices_for_regexp(contents: List[str], str_regexp: str):
        c_regexp = re.compile(str_regexp)
        return [i for i, point in enumerate(contents) if c_regexp.match(point)]
    ...
    def replace_target(self, target_file=None, method="random"):
        '''
        Similar to random target but tuned for vexp
        '''
        if target_file is None:
            target_file = target_file or random.choice(self.target_files)
        assert target_file in self.target_files
        ...
        # Matches all occurences of ./let[1]/match[1]/pair[1-9](one or more occurence of app[1-9], if[1-9], or pexp[1-9]) then nothing after
        # Example--- Matches: ./let[1]/match[1]/pair[1]/pexp[1]/vexp[1] and ./let[1]/match[1]/pair[2]/pexp[1]/vexp[1]
        # Does not match: ./let[1]/match[1]/vexp[1] or ./let[1]/match[1]/pair[2]/app[1]/vexp[1]/let[1]
        # valid_path_regex = re.compile(r'\./let\[1\]/match\[1\]/pair\[\d+\](/pexp|/if|/app|(/if\[\d+\]/next\[\d+\]/pexp))\[\d+\](/vexp\[\d+\])+$')
        valid_indices = self.get_match_indices_for_regexp(self.modification_points[target_file], r'\./let\[1\]/match\[1\]/pair\[\d+\](/pexp|/if|/app|(/if\[\d+\]/next\[\d+\]/pexp))\[\d+\](/vexp\[\d+\])+$')
        assert method in ['random', 'weighted']
        if method == 'random' or target_file not in self.modification_weights:
            return (target_file, random.choice(valid_indices))

    def delete_target(self, target_file=None, method="random"):
        '''
        Similar to random target but tuned for vexp
        '''
        if target_file is None:
        ...
        # Does not match: ./let[1]/match[1]/vexp[1] or ./let[1]/match[1]/pair[2]/app[1]/vexp[1]/let[1]
        # valid_path_regex = re.compile(r'\./let\[1\]/match\[1\]/pair\[\d+\](/pexp|/if|/app|(/if\[\d+\]/next\[\d+\]/pexp))\[\d+\](/vexp\[\d+\])+$')
        valid_indices = self.get_match_indices_for_regexp(self.modification_points[target_file], r'\.(/let\[1\]/match\[1\]/pair\[\d+\](/pexp|/if|/app|(/if\[\d+\]/next\[\d+\]/pexp))\[\d+\]+$|/app\[\d+\]+$|/pexp\[\d+\]+$)')
        assert method in ['random', 'weighted']
        if method == 'random' or target_file not in self.modification_weights:
            return (target_file, random.choice(valid_indices))
